<?php
function initialize_motor_gallery_slick_slider()
{
?>
...
add_action('wp_footer', 'initialize_motor_gallery_slick_slider');

function top_motor_shortcode($atts)
{
    global $wpdb;
    ...
    $term = get_term_by('slug', $make, 'motorcycle_make');
    $term_name = $term ?  $term->name : '';

    $top_bikes = [];
    if ($term) {
        // Get listings where _listing_make matches term ID and state is 1
        $top_bikes = get_posts(array(
    ...
    <?php if (!empty($top_bikes)) : ?>
        <div class="top-cars-sedan-carousel">
            <ul class="top-cars-sedan-list">
                <?php foreach ($top_bikes as $bike_post) : ?>
                    <?php
                    // Set up post data for the current car listing
                    setup_postdata($bike_post);

                    // Get car title, thumbnail GUID, and make terms
                    $car_title = get_the_title($bike_post->ID); // Get the title for the listing
                    // Get the car makes
                    $listing_makes = wp_get_post_terms($bike_post->ID, 'motorcycle_make');
//                     $make_names = wp_list_pluck($listing_makes, 'name');
                    ...
                    $post_thumbnail_id = get_post_thumbnail_id($bike_post->ID);
                    ...
                    <li class="car-sedan-item">
                        <div class="car-thumbnail">
                            <span>
                                <img src="<?php echo $guid; ?>" alt="<?php echo esc_attr($bike_post->post_title); ?>" class="fixed-thumbnail">
                            </span>
                        </div>

                        <div class="car-content">
                            <div class="car-info">
								<p class="car-make"><?php echo $term_name; ?></p>
                            </div>
                            <div class="car-titile-con">
                                <a href="<?php echo get_permalink($bike_post->ID); ?>" class="car-title"><?php echo esc_html($car_title); ?></a>
...
add_shortcode('top_motor', 'top_motor_shortcode');
