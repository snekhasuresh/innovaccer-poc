(function ($) {
	...
	var listingLocation = {
		...
		initializeMap: function(mapInstance) {
			...
			var latLng = [wp_cardealer_maps_opts.default_latitude, wp_cardealer_maps_opts.default_longitude];
			var zoom = 8;
			...
			if ( wp_cardealer_maps_opts.geocoder_country ) {
				geocoder = new L.Control.Geocoder.Nominatim({
					geocodingQueryParams: {countrycodes: wp_cardealer_maps_opts.geocoder_country}
				});
			} else {
				geocoder = new L.Control.Geocoder.Nominatim();
			}
            ...
            if ( wp_cardealer_maps_opts.map_service == 'mapbox' ) {
                var tileLayer = L.tileLayer('https://api.mapbox.com/styles/v1/mapbox/'+wp_cardealer_maps_opts.mapbox_style+'/tiles/{z}/{x}/{y}?access_token='+ wp_cardealer_maps_opts.mapbox_token, {
                    attribution: " &copy;  <a href='https://www.mapbox.com/about/maps/'>Mapbox</a> &copy;  <a href='http://www.openstreetmap.org/copyright'>OpenStreetMap</a> <strong><a href='https://www.mapbox.com/map-feedback/' target='_blank'>Improve this map</a></strong>",
                    maxZoom: 18,
                });
            } else if ( wp_cardealer_maps_opts.map_service == 'here' ) {
                
                var hereTileUrl = 'https://2.base.maps.ls.hereapi.com/maptile/2.1/maptile/newest/'+wp_cardealer_maps_opts.here_style+'/{z}/{x}/{y}/512/png8?apiKey='+ wp_cardealer_maps_opts.here_map_api_key +'&ppi=320';
                var tileLayer = L.tileLayer(hereTileUrl, {
                    attribution: " &copy;  <a href='https://www.mapbox.com/about/maps/'>Here</a> &copy; <strong><a href='https://www.mapbox.com/map-feedback/' target='_blank'>Improve this map</a></strong>",
                    maxZoom: 18,
                });

            } else if ( wp_cardealer_maps_opts.map_service == 'openstreetmap' ) {
                ...
                if ( wp_cardealer_maps_opts.custom_style != '' ) {
                    try {
                        var custom_style = $.parseJSON(wp_cardealer_maps_opts.custom_style);
                        var tileLayer = L.gridLayer.googleMutant({
                            type: wp_cardealer_maps_opts.googlemap_type,
                            styles: custom_style
                        });
            ...
            if ( wp_cardealer_maps_opts.map_service === 'google-map' ) {
				...
				$('.pw-map-search').each(function(){
					...
					if ( wp_cardealer_maps_opts.geocoder_country ) {
	                    autocomplete.setComponentRestrictions({
	                        country: [wp_cardealer_maps_opts.geocoder_country],
	                    });
	                }
