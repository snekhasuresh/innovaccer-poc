...
class WP_CarDealer_Recaptcha {
	...
	public static function enqueue() {
		...
		if ( self::is_recaptcha_enabled() ) {
			wp_enqueue_script( 'recaptcha', '//www.google.com/recaptcha/api.js', array( 'jquery', 'wp-cardealer-main' ), false, true );
		}
	}
	...
	public static function validate_fields( $return ) {
		if ( self::is_recaptcha_enabled() ) {
			...
			if ( !$is_recaptcha_valid ) {
				return new WP_Error( 'validation-error', esc_html__( 'reCAPTCHA is a required field', 'wp-cardealer' ) );
			}
		}
		return $return;
	}
	...
	public static function is_recaptcha_enabled() {
		$site_key = wp_cardealer_get_option( 'recaptcha_site_key' );
		$secret_key = wp_cardealer_get_option( 'recaptcha_secret_key' );
	...
	public static function is_recaptcha_valid( $recaptcha_response ) {
		$response = wp_remote_get(
			add_query_arg(
				array(
					'secret'   => wp_cardealer_get_option( 'recaptcha_secret_key' ),
