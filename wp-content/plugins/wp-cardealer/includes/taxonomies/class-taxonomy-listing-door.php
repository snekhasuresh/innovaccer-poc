...
class WP_CarDealer_Taxonomy_Car_Door{
	...
	public static function definition() {
		$singular = __( 'Door', 'wp-cardealer' );
		$plural   = __( 'Doors', 'wp-cardealer' );
		...
		$rewrite_slug = get_option('wp_cardealer_listing_door_slug');
		if ( empty($rewrite_slug) ) {
			$rewrite_slug = _x( 'listing-door', 'Car door slug - resave permalinks after changing this', 'wp-cardealer' );
		}
		$rewrite = array(
			'slug'         => $rewrite_slug,
			'with_front'   => false,
		...
		register_taxonomy( 'listing_door', 'listing', array(
			'labels'            => apply_filters( 'wp_cardealer_taxomony_listing_door_labels', $labels ),
			'hierarchical'      => false,
			'parent_item'       => null,
...
WP_CarDealer_Taxonomy_Car_Door::init();