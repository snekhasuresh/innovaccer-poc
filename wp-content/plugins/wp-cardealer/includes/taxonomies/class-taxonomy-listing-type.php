...
class WP_ListingDealer_Taxonomy_Listing_Type{
	...
	public static function init() {
		...
		add_filter( "manage_edit-listing_type_columns", array( __CLASS__, 'tax_columns' ) );
		add_filter( "manage_listing_type_custom_column", array( __CLASS__, 'tax_column' ), 10, 3 );
		add_action( "listing_type_add_form_fields", array( __CLASS__, 'add_fields_form' ) );
		add_action( "listing_type_edit_form_fields", array( __CLASS__, 'edit_fields_form' ), 10, 2 );
	...
	public static function definition() {
		$singular = __( 'Type', 'wp-cardealer' );
		$plural   = __( 'Types', 'wp-cardealer' );
		...
		$rewrite_slug = get_option('wp_cardealer_listing_type_slug');
		if ( empty($rewrite_slug) ) {
			$rewrite_slug = _x( 'listing-type', 'Listing type slug - resave permalinks after changing this', 'wp-cardealer' );
		}
		$rewrite = array(
			'slug'         => $rewrite_slug,
			'with_front'   => false,
		...
		register_taxonomy( 'listing_type', 'listing', array(
			'labels'            => apply_filters( 'wp_cardealer_taxomony_listing_type_labels', $labels ),
			'hierarchical'      => false,
			'parent_item'       => null,
	...
	public static function add_fields_form($taxonomy) {
		...
		<div class="form-field">
			<label><?php esc_html_e( 'Select Categories', 'wp-cardealer' ); ?></label>
	...
	public static function edit_fields_form( $term, $taxonomy ) {
		...
		?>
		<tr class="form-field">
			<th scope="row" valign="top"><label><?php esc_html_e( 'Select Categories', 'wp-cardealer' ); ?></label></th>
			<td>
	...
	public static function categories_field( $val = '' ) {
		...
		$terms_hash = 'wpcd_cats_' . md5( wp_json_encode( $args ) . WP_CarDealer_Cache_Helper::get_transient_version('wpcd_get_listing_category') );
