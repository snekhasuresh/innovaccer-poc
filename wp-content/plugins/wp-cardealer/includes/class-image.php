...
class WP_CarDealer_Image {
	
	public static function init() {
		add_filter( 'upload_dir', array( __CLASS__, 'upload_dir' ) );
		// Ajax endpoints.
		add_action( 'wpcd_ajax_wp_cardealer_ajax_upload_file',  array(__CLASS__,'process_upload_file') );
		
		// compatible handlers.
		add_action( 'wp_ajax_wp_cardealer_ajax_upload_file',  array(__CLASS__,'process_upload_file') );
		add_action( 'wp_ajax_nopriv_wp_cardealer_ajax_upload_file',  array(__CLASS__,'process_upload_file') );
	}

	public static function process_upload_file() {
		
		if ( ! is_user_logged_in() ) {
			wp_send_json_error( __( 'You must be logged in to upload files using this method.', 'wp-cardealer' ) );
			return;
		}
		$data = array(
			'files' => array(),
	...
	public static function prepare_uploaded_files( $file_data ) {
		...
		return apply_filters( 'wp_cardealer_prepare_uploaded_files', $files_to_upload );
	}
	...
	public static function upload_file( $file, $args = array() ) {
		...
		$wp_cardealer_upload         = true;
		$wp_cardealer_uploading_file = $args['file_key'];
		$uploaded_file              = new stdClass();

		if ( '' === $args['allowed_mime_types'] ) {
			$allowed_mime_types = self::get_allowed_mime_types($wp_cardealer_uploading_file);
		} else {
	...
	public static function create_attachment( $attachment_url, $post_id = 0 ) {
		...
		$image_sizes_remove = apply_filters( 'wp-cardealer-create-attachment-remove-image-sizes', ['shop_thumbnail', 'shop_single', 'shop_catalog', 'woocommerce_gallery_thumbnail', 'woocommerce_single', 'woocommerce_thumbnail'] );
		...
		$attachment = array(
			'post_title'   => esc_html__('Attachment Image', 'wp-cardealer'),
			'post_content' => '',
			'post_status'  => 'inherit',
			'post_parent'  => $post_id,
			'guid'         => $attachment_url,
	...
	public static function get_allowed_mime_types( $field_key = '' ) {
		...
		if ( !empty($allowed_mime_types) ) {
			...
			$return = WP_CarDealer_Mixes::get_image_mime_types();
		}
		
		return apply_filters( 'wp_cardealer_mime_types', $allowed_mime_types, $field_key );
	}
