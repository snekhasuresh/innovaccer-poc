...
function motor_fuel_consumption_view_model_styles()
{
    wp_enqueue_style('fuel-consumption-view-model-style', get_stylesheet_directory_uri() . '/widget-shortcodes/motor-template-listings/css/motofuel-consumption-view-model.css', array(), '1.0', 'all');
}

add_shortcode('motor_fuel_consumption_view_model', 'motor_fuel_consumption_view_model_shortcode');
function motor_fuel_consumption_view_model_shortcode()
{
    motor_fuel_consumption_view_model_styles();

    ob_start();

    echo motor_display_single_car_post();

    return ob_get_clean();
}

function motor_display_single_car_post()
{
    ...
    $base_url = $base_url . '/motorcycles/' . $make . '/';
        ...
        <div class="car-item">
            <a href="<?php echo esc_url($permalink); ?>" class="car-link">
                ...
                <span class="car-details">
                    <p class="car-make"><?php echo get_the_term_list($post_id, 'motorcycle_make', '', ', '); ?></p>
