...
function enqueue_find_new_cars_comparison_css()
{
    ...
    wp_enqueue_style('find-new-cars-comparison-style', get_stylesheet_directory_uri() . '/widget-shortcodes/new-cars/css/find-new-cars-comparison.css', array(), '1.0', 'all');
}
// add_action('wp_enqueue_scripts', 'enqueue_find_new_cars_comparison_css');

function initialize_findnew_slider()
{
?>
...
add_action('wp_footer', 'initialize_findnew_slider');


function findnew_car_comparison($atts)
{
    enqueue_find_new_cars_comparison_css();
    ...
    ob_start();
    $car_comparison_data = get_car_comparison_data($brand_id);
...
'compare brand' => 'So sánh ô tô ',

];


?>
    <h2 class="wa-title-text find-new-cars-compare-title"> <?php 
		if($brand_id){
			echo esc_html($translate['compare brand']) . ' ' . esc_html($brand_name); 
		}else{
			echo esc_html($translate['Compare']);
		}
		?>
	</h2>
...
add_shortcode('findnew_car_comparison', 'findnew_car_comparison');
