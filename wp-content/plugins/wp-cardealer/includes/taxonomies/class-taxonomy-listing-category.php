...
class WP_CarDealer_Taxonomy_Car_Category{
	...
	public static function definition() {
		$singular = __( 'Category', 'wp-cardealer' );
		$plural   = __( 'Categories', 'wp-cardealer' );
		...
		$rewrite_slug = get_option('wp_cardealer_listing_category_slug');
		if ( empty($rewrite_slug) ) {
			$rewrite_slug = _x( 'listing-category', 'Car category slug - resave permalinks after changing this', 'wp-cardealer' );
		}
		$rewrite = array(
			'slug'         => $rewrite_slug,
			'with_front'   => false,
		...
		register_taxonomy( 'listing_category', 'listing', array(
			'labels'            => apply_filters( 'wp_cardealer_taxomony_listing_category_labels', $labels ),
			'hierarchical'      => false,
			'parent_item'       => null,
...
WP_CarDealer_Taxonomy_Car_Category::init();