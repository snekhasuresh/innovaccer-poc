...
class WP_CarDealer_Taxonomy_Car_Fuel_Type{
	...
	public static function definition() {
		$singular = __( 'Fuel Type', 'wp-cardealer' );
		$plural   = __( 'Fuel Types', 'wp-cardealer' );
		...
		$rewrite_slug = get_option('wp_cardealer_listing_fuel_type_slug');
		if ( empty($rewrite_slug) ) {
			$rewrite_slug = _x( 'listing-fuel-type', 'Car fuel type slug - resave permalinks after changing this', 'wp-cardealer' );
		}
		$rewrite = array(
			'slug'         => $rewrite_slug,
			'with_front'   => false,
		...
		register_taxonomy( 'listing_fuel_type', 'listing', array(
			'labels'            => apply_filters( 'wp_cardealer_taxomony_listing_fuel_type_labels', $labels ),
			'hierarchical'      => false,
			'parent_item'       => null,
...
WP_CarDealer_Taxonomy_Car_Fuel_Type::init();