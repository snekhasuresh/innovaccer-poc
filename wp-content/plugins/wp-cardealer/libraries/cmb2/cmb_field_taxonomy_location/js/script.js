(function ($) {
	...
	$('.wpcd_taxonomy_location').each(function () {
	...
	$('.cmb-repeatable-group').on('cmb2_add_group_row_start', function (event, instance) {
		...
		$oldRow.find('.wpcd_taxonomy_location').each(function () {
	...
	$('.cmb-repeatable-group').on('cmb2_add_row', function (event, newRow) {
		$(newRow).find('.wpcd_taxonomy_location').each(function () {
		...
		$(newRow).prev().find('.wpcd_taxonomy_location').each(function () {
			var allowclear = $(this).data('allowclear');
			var width = $(this).data('width') ? $(this).data('width') : '100%';
			$(this).select2({
				allowClear: allowclear,
	...
	$('.cmb-repeatable-group').on('cmb2_shift_rows_start', function (event, instance) {
		...
		groupWrap.find('.wpcd_taxonomy_location').each(function () {
			$(this).select2('destroy');
		});
	...
	// Location Change
    $('body').on('change', 'select.wpcd_taxonomy_location', function(){
        var val = $(this).val();
        var next = $(this).data('next');
        var main_select = 'select.wpcd_taxonomy_location' + next;
        if ( $(main_select).length > 0 ) {
            ...
            if ( val ) {
                ...
                $.ajax({
                    url: location_opts.ajaxurl_endpoint.toString().replace( '%%endpoint%%', 'wpcd_process_change_location' ),
                    type:'POST',
                    dataType: 'json',
                    data:{
