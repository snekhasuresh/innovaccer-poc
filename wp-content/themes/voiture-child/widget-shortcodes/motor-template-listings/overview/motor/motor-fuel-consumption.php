...
function overview_motor_fuel_consumption()
{
    ...
    if (!empty($variant_posts)) {
        foreach ($variant_posts as $variant_post) {
            ...
            $gear_box = isset($variant_meta['gearbox'][0]) ? $variant_meta['gearbox'][0] : '';
            $claim = isset($variant_meta['fuel_consumption'][0]) ? $variant_meta['fuel_consumption'][0] : '';

            // Add each variant's data to the table_data array
            if (!empty($fuel_type) && !empty($gear_box) && !empty($claim)) {
                $table_data[] = [
                    'fuel_type' => $fuel_type,
                    'gear_box' => $gear_box,
                    'claim' => $claim
...
?>
        <div>
            <div class="individula-fuel-title-con">
                <h2 class="individula-fuel-title wa-title-text"><?php echo esc_html('อัตราสิ้นเปลืองเชื้อเพลิง ' . $post_title); ?></h2>
            </div>
                ...
                <tbody>
                    <?php foreach ($table_data as $index => $row): ?>
                        <tr class="table-row <?php echo $index >= 3 ? 'hidden-row' : ''; ?>"
                            ...
                            <td><?php echo esc_html($row['gear_box']); ?></td>
...
add_shortcode('overview_motor_fuel_consumption', 'overview_motor_fuel_consumption');
