<?php
function render_news_tab_content($query_args, $is_ajax = false, $page = 1, $posts_per_page = 10)
{
...
function news_single_listing_shortcode($atts)
{
    ob_start();
    ...
    if ($latest_query->have_posts()) :
        while ($latest_query->have_posts()) : $latest_query->the_post();
            ...
            if ($news_category) {
                $category = get_term($news_category[0], 'news-category');
                if ($category->parent && $category->parent == 0) {
                    $news_categories[] = $news_category[0];
                } else {
                    $get_parent_terms = get_term($category->parent, 'news-category');
        ...
        foreach (array_unique($news_categories) as $term_id) {
            $term = get_term($term_id, 'news-category');
...
?>
            ...
            <ul class="single-listing-tabs-nav">
                ...
                <?php if (!empty($category_names)) :
                    ...
                    foreach ($category_names as $category) :
                        ...
                        WHERE t.parent = %d
                        AND t.taxonomy = %s
                        ", $category->term_id, 'news-category');
    ...
    <div id="latest" class="single-listing-tab-content">
        <ul class="news-list" data-tab-id="latest">
            ...
            echo render_news_tab_content($latest_args);
            ?>
        </ul>
        <?php
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function loadNews(termId, targetTab, subcategories, page) {
                ...
                data.append('action', 'load_category_news');
            ...
            document.querySelectorAll('.view-more').forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    const data = new FormData();
                    data.append('action', 'load_more_individual_news');
                    data.append('page', page);
...
add_shortcode('news_single_listing', 'news_single_listing_shortcode');

add_action('wp_ajax_load_more_individual_news', 'load_more_individual_news');
add_action('wp_ajax_nopriv_load_more_individual_news', 'load_more_individual_news');

add_action('wp_ajax_load_category_news', 'load_category_news');
add_action('wp_ajax_nopriv_load_category_news', 'load_category_news');

function load_more_individual_news()
{
    ...
    $news_content = render_news_tab_content($args, true, $paged);
    echo $news_content;
    die();
}

function load_category_news()
{
    $paged = isset($_POST['page']) ? intval($_POST['page']) : 1;
    $post_id = intval($_POST['post_id']);
    $subcategories = isset($_POST['subcategories']) ? explode(',', $_POST['subcategories']) : [];
    ...
    echo render_news_tab_content($args, true, $paged, $posts_per_page);
}
