...
function single_listing_motor_variants()
{
	...
	if ($highest_price == $lowest_price && $highest_price != 0) {
		...
		$price = format_price_vietnam($highest_price);
	} elseif ($lowest_price != 0 && $highest_price != 0) {
            ...
            <div id="listing-detail-description" class="description inner col-md-12">
                ...
                <div class="tabs-container-varient">
                    <?php if (!empty($variants_on_sale)) { ?>
                        <div class="price-dec"><?php 
                        echo sprintf(
                            'The %s %s is offered in %d variants - which are priced from %s to %s, the base model of %s is %s which is at a price of %s and the top variant of %s is %s which is offered at a price of %s.',
                            $year_range,
                            $post_title,
                            $variant_on_sale_count,
                    ...
                    <div class="custom-tabs-header">
                        ...
                        <?php if (!empty($variants_not_on_sale)) { ?>
                            <button class="custom-tab-btn  <?php echo (!empty($variants_not_on_sale) && empty($variants_on_sale)) ? 'custom-tab-active' : '' ?>" data-tab="tab-not-on-sale">
                                <?php esc_html_e('Các biến thể không được bán', 'your-textdomain'); ?>
                            </button>
                        <?php } ?>
                    </div>
                    <?php if (!empty($variants_on_sale)) { ?>
                            ...
                            <div class="car-list-varient">
                                <?php foreach ($variants_on_sale as $variant) {
                                    ...
                                    $grouped_variants[$group_key][] = [
                                        ...
                                        'retail_price' => (isset($meta_data['price'][0]) && $meta_data['price'][0] != 0) ? format_price_vietnam($meta_data['price'][0]) : 'Đang cập nhật',
                                ...
                                ?>
                                    ...
                                    <?php foreach ($variants as $variant) { ?>
                                        <div class="car-item-c">
                                            ...
                                            $variant_url = $base_url . '/xe-may/' . $make . '/' . $model . '/' . $variant['post_name'];
                                            ?>
                                            ...
                                            <div class="right-content">
                                                ...
                                                <a href="<?php echo home_url('dung-cu/mua-xe-tra-gop'); ?>" class="calculator-icon">
                                                ...
                                                <div class="actions">
                                                    <a href="<?php echo home_url('so-sanh-xe-may'); ?>" class="compare-btn"><?php esc_html_e('+ So sánh  ', 'your-textdomain'); ?></a>
                                                </div>
                                            </div>
                                        </div>
                                    <?php } ?>
