<?php
function enqueue_bike_brands_css()
{
    // Register and enqueue the CSS file
    wp_enqueue_style('bike-brands-style', get_stylesheet_directory_uri() . '/widget-shortcodes/popular-car-brands/brands-in-my.css', array(), '1.0', 'all');
}
add_action('wp_enqueue_scripts', 'enqueue_bike_brands_css');

// Shortcode function to display popular car brands
function display_popular_bike_brands()
{
    ob_start(); // Start output buffering
    // $cache_key = 'brands_list';
    // $sorted_brands = get_transient($cache_key);
    // if (false === $sorted_brands) {
    $bike_brands = get_terms(array(
        'taxonomy' => 'motorcycle_make',
        'hide_empty' => false, // Set to true to hide empty terms
    ));

    $sorted_brands = [];
    if (!is_wp_error($bike_brands) && !empty($bike_brands)) {
        $brands_with_sort = [];

        // Filter brands based on their 'state' and sort by 'sort_order'
        foreach ($bike_brands as $brand) {
...
?>
    ...
    <div class="car-brands">
        <?php foreach ($sorted_brands as $brand):
            // Assuming you have a custom field 'motorcycle_make_image' for the logo URL
...
add_shortcode('popular_bike_brands', 'display_popular_bike_brands');
?>