...
function enqueue_find_new_cars_css()
{
                    ...
                    <?php foreach ($popular_cars_in_malaysia as $key => $value) : ?>
                        <li>
                            <a
                                ...
                                onclick="changeTab('<?php echo str_replace(' ', '_', $key); ?>-content')"
            ...
            ?>
                <?php foreach ($popular_cars_in_malaysia as $key => $value) : ?>
                    <div id="<?php echo str_replace(' ', '_', $key); ?>-content" class="tab-pane <?php echo $first_pane ? 'active' : ''; ?>">
                        <?php display_popular_car_posts($value, $brand_id); ?>
                    </div>
...
add_shortcode('popular_cars_in_new_cars', 'popular_cars_in_new_cars');

function popular_car_tabs_inline_script()
{
    enqueue_find_new_cars_css();
?>
    <script>
...
add_action('wp_footer', 'popular_car_tabs_inline_script');

function display_popular_car_posts($cars, $brand_id = 0)
{
    enqueue_find_new_cars_css();
    $posts_per_page = 9;
    ...
    $top_car_model_ids = get_all_top_car_model_ids_data(); //fetch_all_top_car_model_ids_from_db();
    ...
    if (!empty($cars)) {
        ...
        foreach ($cars as $index => $car) {
            ...
            if (in_array($car_id, $top_car_model_ids)) {
                $is_hot = true;
            }
            ...
            echo '<span><img src="' . esc_url($guid) . '" alt="' . esc_attr($car_title) . '" loading="lazy"></span>';
