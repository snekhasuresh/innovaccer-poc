...
class WP_CarDealer_CMB2_Field_File {

	public static function init() {
		add_filter( 'cmb2_render_wp_cardealer_file', array( __CLASS__, 'render_map' ), 10, 5 );
		add_filter( 'cmb2_sanitize_wp_cardealer_file', array( __CLASS__, 'sanitize_map' ), 10, 4 );	
		add_filter( 'cmb2_types_esc_wp_cardealer_file', array( __CLASS__, 'esc_escaped_value' ), 10, 4 );	
	}
	...
	public static function render_map( $field, $field_escaped_value, $field_object_id, $field_object_type, $field_type_object ) {
		...
		if ( is_user_logged_in() && $ajax ) {
			wp_enqueue_script( 'wp-cardealer-ajax-file-upload' );
			$classes[] = 'wp-cardealer-file-upload';
		}
		...
		?>
		<div class="wp-cardealer-uploaded-files">
			<?php if ( ! empty( $files ) ) : ?>
				<?php if ( is_array( $files ) ) :?>
					<?php foreach ( $files as $key => $value ) : ?>
						<?php
							echo WP_CarDealer_Template_Loader::get_template_part( 'misc/uploaded-file-html', array( 'input_name' => $current_name, 'value' => $key, 'file_url' => $value, 'field' => $field ) );
						?>
					<?php endforeach; ?>
				<?php elseif ( $value = $files ) : ?>
					<?php echo WP_CarDealer_Template_Loader::get_template_part( 'misc/uploaded-file-html', array( 'input_name' => $current_name, 'value' => $value, 'field' => $field ) ); ?>
				<?php endif; ?>
			<?php endif; ?>
		</div>
		...
		<div class="label-can-drag">
			<div class="form-group group-upload">
		        <div class="upload-file-btn">
		            <?php if ( $multiple ) { ?>
		            	<span><?php esc_html_e('Upload Files', 'wp-cardealer'); ?></span>
	            	<?php } else { ?>
		            	<span><?php esc_html_e('Upload File', 'wp-cardealer'); ?></span>
	            	<?php } ?>
				...
				if ( is_array($values) ) {
					...
					foreach ($values as $attachment_url) {
						if ( ! empty( $attachment_url ) && is_numeric( $attachment_url ) ) {
							...
							$attach_id = WP_CarDealer_Image::create_attachment($attachment_url, $object_id);
							if ( $attach_id ) {
					...
					if ( is_numeric( $values ) ) {
						...
						$attach_id = WP_CarDealer_Image::create_attachment( $values, $object_id );
						$url = wp_get_attachment_url( $attach_id );
						$return = $url;
					}
				}
...
WP_CarDealer_CMB2_Field_File::init();