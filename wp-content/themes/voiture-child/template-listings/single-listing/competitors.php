<?php
function enqueue_overview_competitors_css()
{
...
function car_competitors_shortcode()
{
    enqueue_overview_competitors_css();
    ...
    $competitor_post_ids = get_distinct_variants_by_price($min_price, $max_price, $post_id);

    if (empty($competitor_post_ids)) {
        return;
    }

    $car_data = format_car_response($competitor_post_ids);
...
add_shortcode('car_competitors', 'car_competitors_shortcode');

function get_distinct_variants_by_price($min_price, $max_price, $post_id)
{
    global $wpdb;
    $query = $wpdb->prepare(
