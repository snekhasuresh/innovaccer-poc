...
class WP_CarDealer_Taxonomy_Car_Make{
	...
	public static function definition() {
		$singular = __( 'Make', 'wp-cardealer' );
		$plural   = __( 'Makes', 'wp-cardealer' );
		...
		$rewrite_slug = get_option('wp_cardealer_listing_make_slug');
		if ( empty($rewrite_slug) ) {
			$rewrite_slug = _x( 'listing-make', 'Car make slug - resave permalinks after changing this', 'wp-cardealer' );
		}
		$rewrite = array(
			'slug'         => $rewrite_slug,
			'with_front'   => false,
		...
		register_taxonomy( 'listing_make', 'listing', array(
			'labels'            => apply_filters( 'wp_cardealer_taxomony_listing_make_labels', $labels ),
			'hierarchical'      => false,
			'parent_item'       => null,
...
WP_CarDealer_Taxonomy_Car_Make::init();