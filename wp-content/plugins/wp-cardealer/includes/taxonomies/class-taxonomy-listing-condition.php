...
class WP_CarDealer_Taxonomy_Car_Condition{
	...
	public static function init() {
		...
		add_filter( "manage_edit-listing_condition_columns", array( __CLASS__, 'tax_columns' ) );
		add_filter( "manage_listing_condition_custom_column", array( __CLASS__, 'tax_column' ), 10, 3 );

		add_action( "listing_condition_add_form_fields", array( __CLASS__, 'add_fields_form' ) );
		add_action( "listing_condition_edit_form_fields", array( __CLASS__, 'edit_fields_form' ), 10, 2 );
	...
	public static function definition() {
		$singular = __( 'Condition', 'wp-cardealer' );
		$plural   = __( 'Conditions', 'wp-cardealer' );
		...
		$rewrite_slug = get_option('wp_cardealer_listing_condition_slug');
		if ( empty($rewrite_slug) ) {
			$rewrite_slug = _x( 'listing-condition', 'Car condition slug - resave permalinks after changing this', 'wp-cardealer' );
		}
		$rewrite = array(
			'slug'         => $rewrite_slug,
			'with_front'   => false,
		...
		register_taxonomy( 'listing_condition', 'listing', array(
			'labels'            => apply_filters( 'wp_cardealer_taxomony_listing_condition_labels', $labels ),
			'hierarchical'      => false,
			'parent_item'       => null,
...
WP_CarDealer_Taxonomy_Car_Condition::init();