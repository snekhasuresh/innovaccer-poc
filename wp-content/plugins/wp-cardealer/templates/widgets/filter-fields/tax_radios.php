...
$r['lang'] = apply_filters( 'wp-cardealer-current-lang', null );

$categories_hash = 'wpcd_cats_' . md5( wp_json_encode( $r ) . WP_CarDealer_Cache_Helper::get_transient_version('wpcd_get_' . $r['taxonomy']) );
...
$output = '';
if ( ! empty( $categories ) ) {
    include_once WP_CARDEALER_PLUGIN_DIR . '/includes/walkers/class-category-radio-check-walker.php';

    $walker = new WP_CarDealer_Category_Radio_Check_Walker();
...
?>
    <div class="form-group form-group-<?php echo esc_attr($key); ?> <?php echo esc_attr(!empty($field['toggle']) ? 'toggle-field' : ''); ?> <?php echo esc_attr(!empty($field['hide_field_content']) ? 'hide-content' : ''); ?> tax-radios-field tax-viewmore-field">
        ...
        <div class="form-group-inner">
            ...
            <a class="toggle-filter-viewmore" href="javascript:void(0);"><span class="icon-more"><i class="ti-plus"></i></span> <span class="text"><?php esc_html_e('Show More', 'wp-cardealer'); ?></span></a>
        </div>
    </div><!-- /.form-group -->
<?php }