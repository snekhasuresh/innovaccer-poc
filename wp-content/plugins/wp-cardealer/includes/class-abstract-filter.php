...
class WP_CarDealer_Abstract_Filter {
		...
		if ( ! empty( $params['filter-orderby'] ) ) {
			switch ( $params['filter-orderby'] ) {
				...
				case 'published':
					...
					break;
				case 'price-lowest':
					$query_vars['meta_key'] = WP_CARDEALER_LISTING_PREFIX . 'price';
	...
	public static function get_posts_by_distance($latitude, $longitude, $distance) {
		...
		if ( apply_filters( 'wp_cardealer_get_listings_cache_results', false ) ) {
			$to_hash         = json_encode( array($earth_distance, $latitude, $longitude, $latitude, $distance, $post_stype) );
			$query_args_hash = 'wp_cardealer_' . md5( $to_hash . WP_CARDEALER_PLUGIN_VERSION );
	...
	public static function filter_count($name, $term_id, $field) {
		...
		$cached_counts = (array) get_transient( 'wp_cardealer_filter_counts' );
		if ( ! isset( $cached_counts[ $query_hash ] ) ) {
			$loop = WP_CarDealer_Query::get_posts($args, $params);
			$cached_counts[ $query_hash ] = $loop->found_posts;
			set_transient( 'wp_cardealer_filter_counts', $cached_counts, DAY_IN_SECONDS );
		}
	...
	public static function filter_get_name($key, $field) {
		...
		$name = $prefix.'-'.$key;
		return apply_filters('wp-cardealer-filter-get-name', $name, $key, $field);
	}
	...
	public static function filter_field_input($instance, $args, $key, $field) {
		...
		include WP_CarDealer_Template_Loader::locate( 'widgets/filter-fields/text' );
	}
	...
	public static function filter_date_field_input($instance, $args, $key, $field) {
		...
		include WP_CarDealer_Template_Loader::locate( 'widgets/filter-fields/text_date' );
	}
	...
	public static function filter_field_input_location($instance, $args, $key, $field) {
		...
		include WP_CarDealer_Template_Loader::locate( 'widgets/filter-fields/text_location' );
	}
    
    public static function filter_field_input_distance($instance, $args, $key, $field) {
		$name = self::filter_get_name($key, $field);
		$selected = !empty( $_GET[$name] ) ? $_GET[$name] : apply_filters( 'wp_cardealer_filter_distance_default', 50 );
	...
	public static function filter_field_year_built_range_slider($instance, $args, $key, $field) {
		$name = self::filter_get_name($key, $field);
		$selected = !empty( $_GET[$name] ) ? $_GET[$name] : '';
		$min_max = WP_CarDealer_Query::get_min_max_meta_value(WP_CARDEALER_LISTING_PREFIX.'year_built', 'listing');
		if ( empty($min_max) ) {
			return;
		}
		$min    = !empty($min_max->min) ? floor( $min_max->min ) : 0;
	...
	public static function filter_field_listing_price($instance, $args, $key, $field) {
		...
		$price_min = WP_CarDealer_Query::get_min_max_meta_value(WP_CARDEALER_LISTING_PREFIX.'price', 'listing');
		$price_max = WP_CarDealer_Query::get_min_max_meta_value(WP_CARDEALER_LISTING_PREFIX.'max_price', 'listing');
	...
	public static function filter_field_checkbox($instance, $args, $key, $field) {
		...
		include WP_CarDealer_Template_Loader::locate( 'widgets/filter-fields/checkbox' );
	}
	...
	public static function filter_field_select($instance, $args, $key, $field) {
		...
		include WP_CarDealer_Template_Loader::locate( 'widgets/filter-fields/select' );
	}
	...
	public static function filter_field_taxonomy_radio_list($instance, $args, $key, $field) {
		...
		include WP_CarDealer_Template_Loader::locate( 'widgets/filter-fields/radios' );
	}
	...
	public static function filter_field_taxonomy_check_list($instance, $args, $key, $field) {
		...
		include WP_CarDealer_Template_Loader::locate( 'widgets/filter-fields/check_list' );
	}
	...
	public static function filter_field_taxonomy_hierarchical_radio_list($instance, $args, $key, $field) {
	    ...
	    include WP_CarDealer_Template_Loader::locate( 'widgets/filter-fields/tax_radios' );
	}
	...
	public static function filter_field_taxonomy_hierarchical_check_list($instance, $args, $key, $field) {
	    ...
	    include WP_CarDealer_Template_Loader::locate( 'widgets/filter-fields/tax_check_list' );
	}
	...
	public static function filter_field_taxonomy_hierarchical_select($instance, $args, $key, $field) {
	    ...
	    include WP_CarDealer_Template_Loader::locate( 'widgets/filter-fields/tax_select' );
	}
	...
	public static function filter_field_location_select($instance, $args, $key, $field) {
	    ...
	    if ( $location_type === 'no' ) {
	    	...
	    	include WP_CarDealer_Template_Loader::locate( 'widgets/filter-fields/regions_select' );
	    }
	}
	...
	public static function hierarchical_tax_tree($catId, $depth, $input_name, $key, $field, $selected, $input_type = 'checkbox') {
		...
		if ( ! is_wp_error( $terms ) && ! empty ( $terms ) ) {
			$_id = WP_CarDealer_Mixes::random_key();
			foreach ($terms as $term) {
			  	$checked = '';
			  	if ( !empty($selected) ) {
	...
	public static function filter_field_min_max_input($instance, $args, $key, $field) {
	    ...
	    include WP_CarDealer_Template_Loader::locate( 'widgets/filter-fields/min_max_text' );
	}

	public static function filter_field_range_slider($instance, $args, $key, $field) {
		$name = self::filter_get_name($key, $field);
		$selected = !empty( $_GET[$name] ) ? $_GET[$name] : '';
		$min_max = WP_CarDealer_Query::get_min_max_meta_value($field['id'], 'listing');
		if ( empty($min_max) ) {
			return;
		}
	...
	public static function filter_field_number_select($instance, $args, $key, $field) {
	    ...
	    include WP_CarDealer_Template_Loader::locate( 'widgets/filter-fields/number_select' );
	}
