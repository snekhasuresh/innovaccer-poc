...
var wp_cardealer_all_loaded_icons = {};
jQuery(document).ready(function($){
    
    // custom fields
    $(document).on('click', '.wp-cardealer-custom-field-add-available-field', function () {
        "use strict";
        ...
        var ajax_url = wp_cardealer_customfield_common_vars.ajax_url,
                $this = $(this),
        ...
        var action = 'wp_cardealer_custom_field_available_html';
    ...
    $(document).on('click', '.btn-add-field', function () {
        ...
        var ajax_url = wp_cardealer_customfield_common_vars.ajax_url,
                $this = $(this),
                randid = $this.data('randid'),
                select_element = $('.wp-cardealer-field-types'),
        ...
        action = 'wp_cardealer_custom_field_html';
        var prefix = $(this).closest('form').data('prefix');
        var dataString = 'action=' + action + '&fieldtype=' + fieldtype + '&global_custom_field_counter=' + global_custom_field_counter + '&prefix=' + prefix;
    ...
    $(document).on('click', '.custom-fields-remove', function () {
        ...
        $('.custom-field-class-' + randid).slideUp("normal").promise().done(function () {
            $(this).remove();
            $('.wp-cardealer-form-field-list ul li.'+ fieldtype).removeClass('disabled');
    ...
    $(document).on('keyup change', '.wp-cardealer-custom-field-label', function(e){
        var parent = $(this).parents('.wp-cardealer-custom-field-container');
        parent.find('.field-intro a b').text( '(' + $(this).val() + ')' );
    });

    $(document).on('keyup change', '.wp-cardealer-custom-field-key', function(e){
        if (!wp_cardealer_isArrowKey(e)) {
            var new_val = wp_cardealer_str2url($(this).val().replace(/^[0-9]+\./, ''), 'UTF-8');
            $(this).val(new_val);
            var check = wp_cardealer_check_key_available(new_val);
            if ( check ) {
    ...
    $(document).on('keyup change', 'input[data-general-review-key]', function(e){
        if (!wp_cardealer_isArrowKey(e)) {
            ...
            var new_val = wp_cardealer_str2url($(this).val().replace(/^[0-9]+\./, ''), 'UTF-8');
            $(this).val(new_val);
            var check = wp_cardealer_check_review_key_available(new_val, $('input[data-general-review-key='+cat+']'));
            if ( check ) {
                $(this).parent().find('.error').remove();
                $(this).parent().append('<span class="error">This key is not right</span>');
            } else {
    ...
    function wp_cardealer_isArrowKey(k_ev)
    {
    ...
    function wp_cardealer_str2url(str, encoding, ucfirst)
    {
        str = str.toUpperCase();
        str = str.toLowerCase();
    ...
    function wp_cardealer_check_key_available($key) {
        var $i = 0;
        $('.wp-cardealer-custom-field-key').each(function(){
            if ( $key === $(this).val() ) {
                $i = $i + 1;
            }
        });
    ...
    function wp_cardealer_check_review_key_available($key, $input_e) {
        var $i = 0;
    ...
    // icons
    $(function(){
        $("body").trigger("wp_cardealer_all_loading_icons");
    });
