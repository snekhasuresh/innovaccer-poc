...
function enqueue_motor_color_css()
{
    wp_enqueue_style('car_color-style', get_stylesheet_directory_uri() . '/widget-shortcodes/motor-template-listings/overview/motor/css/car-color.css');
}

function get_motor_colors_data()
{
    enqueue_motor_color_css();
    ...
    <div>
        <div class=" individual-color-title-con">
            <?php if ($is_car_colors_page) : ?>
                <h1 class="individual-color-title"><?php echo esc_html('สี ' . $post_title); ?></h1>
            <?php else : ?>
                <span class="individual-color-title"><?php echo esc_html('สี ' . $post_title); ?></span>
            <?php endif; ?>
        </div>
 	...
 	<div class="view-more-container">
        <a href="<?php echo esc_url(parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH) . '/colors'); ?>" class="view-more-button">
...
add_shortcode("single_listing_motor_color", "get_motor_colors_data")
?>