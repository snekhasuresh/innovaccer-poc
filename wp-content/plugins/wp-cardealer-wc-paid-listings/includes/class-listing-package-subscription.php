...
class WP_CarDealer_Wc_Paid_Listings_Listing_Package_Subscription {
			...
			if ( $post_type === 'listing' ) {
				...
				if ( $package_product_id ) {
					...
					if ( 'listing' === $subscription_type ) {
						$new_count = get_post_meta($user_package_id, WP_CARDEALER_WC_PAID_LISTINGS_PREFIX.'package_count', true);
						$new_count --;

						update_post_meta($user_package_id, WP_CARDEALER_WC_PAID_LISTINGS_PREFIX.'package_count', $new_count);
					}
				}
			}
		}
			...
			if ( 'listing' === $post_type ) {
				...
				if ( $package_product_id ) {
					...
					if ( 'listing' === $subscription_type ) {
						$new_count = get_post_meta($user_package_id, WP_CARDEALER_WC_PAID_LISTINGS_PREFIX.'package_count', true);
						$new_count++;
						$listing_limit = get_post_meta($user_package_id, WP_CARDEALER_WC_PAID_LISTINGS_PREFIX.'listing_limit', true);
	...
	public static function subscription_ended( $subscription ) {
			...
			if ( $user_packages ) {
				foreach ($user_packages as $user_package_id) {
					...
					if ( $package_type == 'listing_package' ) {
						...
						if ( 'listing' === $subscription_type ) {
							$listing_ids = WP_CarDealer_Wc_Paid_Listings_Mixes::get_listings_for_package( $user_package_id  );
		...
		delete_post_meta( $subscription->get_id(), 'wp_cardealer_wc_paid_listings_packages_processed' );
	}
	...
	public static function subscription_activated( $subscription ) {
		...
		if ( get_post_meta( $subscription->get_id(), 'wp_cardealer_wc_paid_listings_packages_processed', true ) ) {
			return;
		}
		...
		foreach ( $subscription->get_items() as $item ) {
			...
			if ( $product->is_type( array( 'listing_package_subscription' ) ) && $subscription->get_user_id() && ! isset( $item['switched_subscription_item_id'] ) ) {
				...
				for ( $i = 0; $i < $item['qty']; $i ++ ) {
					$user_package_id = WP_CarDealer_Wc_Paid_Listings_Mixes::create_user_package( $subscription->get_user_id(), $product->get_id(), $subscription->get_id() );
				}
				...
				foreach ( $listing_ids as $listing_id ) {
					if ( in_array( get_post_status( $listing_id ), array( 'pending_payment', 'expired' ) ) ) {
						WP_CarDealer_Wc_Paid_Listings_Mixes::approve_listing_with_package( $listing_id, $subscription->get_user_id(), $user_package_id );
		...
		update_post_meta( $subscription->get_id(), 'wp_cardealer_wc_paid_listings_packages_processed', true );
	}
			...
			if ( 'package' === $subscription_type ) {
				if ( $user_packages ) {
					...
					if ( $product->get_type() == 'listing_package_subscription' ) {
						WP_CarDealer_Wc_Paid_Listings_Mixes::create_user_package( $subscription->get_user_id(), $item['product_id'], $subscription->get_id() );
					}
				}
				...
				if ( $user_packages ) {
					foreach ( $user_packages as $user_package_id ) {
						...
						if ( $package_type == 'listing_package' ) {
							...
							if ( $listing_ids = WP_CarDealer_Wc_Paid_Listings_Mixes::get_listings_for_package( $user_package_id ) ) {
								foreach ( $listing_ids as $listing_id ) {
									// Featured or not
		...
		if ( $user_packages ) {
			...
			foreach ($user_packages as $user_package_id) {
				...
				if ( $package_type == 'listing_package' ) {
					...
					$switching_to_package_id = WP_CarDealer_Wc_Paid_Listings_Mixes::create_user_package( $user_id, $new_subscription->product_id, $new_subscription->id );
					...
					if ( 'listing' === $new_subscription->type && $switching_to_package_id ) {
						...
						foreach ( $listing_ids as $listing_id ) {
							...
							if ( ! $is_upgrade ) {
								...
								WP_CarDealer_Wc_Paid_Listings_Mixes::increase_package_count( $user_id, $switching_to_package_id );
								// Change the user package ID and package ID
								update_post_meta( $listing_id, WP_CARDEALER_LISTING_PREFIX.'user_package_id', $switching_to_package_id );
								update_post_meta( $listing_id, WP_CARDEALER_LISTING_PREFIX.'package_id', $new_subscription->product_id );
							}

							// Featured or not
							update_post_meta( $listing_id, WP_CARDEALER_LISTING_PREFIX.'featured', $featured );
							// Fire action
							do_action( 'wp_cardealer_wc_paid_listings_switched_subscription', $listing_id, $user_package_id );
						}
					}
				}
			}
...
WP_CarDealer_Wc_Paid_Listings_Listing_Package_Subscription::init();