...
function motor_variant_engine_and_other_images_shortcode()
{
    global $wpdb;
    ...
    foreach ($data as $item) {
        if ($item->type === 'Others') {
            ...
            foreach ($image_data as $image) {
                $other_images[] = [
                    'src' => $image->url,
                    'alt' => $current_variant_post_title . ' Others ' . str_pad($othersIndex++, 3, '0', STR_PAD_LEFT),
                    'title' => $current_variant_post_title . ' Others ' . str_pad($othersIndex - 1, 3, '0', STR_PAD_LEFT),
    ...
    ob_start();
?>
    <h2 class="wa-title-text"><?php esc_html_e($current_variant_post_title . ' Engine & Other Images', 'voiture'); ?></h2>
    ...
    <style>
        ...
        .other-img-view-more svg {
            width: 18px;
            height: 20px;
        }

        .other-img-gallery-list {
            display: flex;
            flex-wrap: wrap;
            gap: 20px;
        }

        .other-img-gallery-list-item {
        ...
        .other-img-gallery-list-item img {
            width: 100%;
            height: 165px;
            display: block;
        ...
        .other-img-description-link:hover {
            color: #ffb400;
        }

        .other-img-view-more-container {
            text-align: center;
            margin-top: 20px;
        }

        .other-img-view-more-link {
            padding: 10px 20px;
        ...
        .other-img-view-more-link:hover {
            background-color: #ffb400;
        }
    </style>
<?php
...
add_shortcode('motor_variant_engine_and_other_images', 'motor_variant_engine_and_other_images_shortcode');
