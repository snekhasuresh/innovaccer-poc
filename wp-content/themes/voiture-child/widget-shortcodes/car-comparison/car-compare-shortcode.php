...
function populate_compare_cars()
{
    global $wpdb;
...
?>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
                            ...
                            success: function(response) {
                                if (response.success && response.data.listings) {
                                    ...
                                    const promises = response.data.listings.map((listing, index) => {
                                        ...
                                        return compare_cars_from_url(modelId, variantId, containers[index]);
                                    });
                                    ...
                                    Promise.all(promises)
                                        .then(() => {
                                            ...
                                            updateComparison(selectedVariants);
                                        })
                                        .catch(error => {
            ...
            function generateReviewsJsonLd(listingName, reviews) {
                let totalRating = 0;
            ...
            function compare_cars_from_url(modelId, variantId, container) {
                    ...
                    jQuery.ajax({
                        ...
                        success: function(response) {
                            if (response.success) {
                                ...
                                if (json_ld) {
                                    generateReviewsJsonLd(json_ld['title'], json_ld['user_reviews']);
                                }
            ...
            document.querySelectorAll('.selects-car').forEach(container => updateContainer(container));
                                ...
                                xhr.onload = function() {
                                    if (xhr.status === 200) {
                                        ...
                                        if (response.success) {
                                            ...
                                            if (newVariantData.variant_data['retail_price']) {
            ...
            function updateComparison(selectedVariants) {
                ...
                if (selectedCars.length < 2) {
                            ...
                            if (selectedCars[i]) {
                                ...
                                if (spec.source === 'car') {
                                    ...
                                    if (spec.key === 'aggregated_user_rating') {
                                        ...
                                        value = rating + '/5 ' + displayStars(rating);
                                    } else {
                    ...
                    function createCarDimensionRow(selectedCars) {
                        const row = document.createElement('tr');
                    ...
                    const dimensionRow = createCarDimensionRow(selectedCarDimensions);
                    // Append to your table body
                    document.querySelector('tbody').appendChild(dimensionRow);

                    function createPhotoComparisonRows(selectedCars) {
                        const rows = [];
                    ...
                    const [exteriorRow, interiorRow] = createPhotoComparisonRows(selectedCarPhotos);
...
add_shortcode('compare_cars_variant', 'selects_cars_shortcode');


add_action('wp_ajax_compare_cars_action', 'handle_compare_cars_ajax');
add_action('wp_ajax_nopriv_compare_cars_action', 'handle_compare_cars_ajax');

function handle_compare_cars_ajax()
{
...
add_action('wp_ajax_get_compare_brands', 'get_compare_brands');

// Register AJAX action for non-logged-in users (if needed)
add_action('wp_ajax_nopriv_get_compare_brands', 'get_compare_brands');
...
add_action('wp_ajax_get_compare_models', 'get_compare_models');
add_action('wp_ajax_nopriv_get_compare_models', 'get_compare_models');

function get_compare_models()
{
    global $wpdb;
...
add_action('wp_ajax_get_compare_variants', 'get_compare_variants');
add_action('wp_ajax_nopriv_get_compare_variants', 'get_compare_variants');


function get_compare_variants()
{
    global $wpdb;
...
add_action('wp_ajax_get_variant_details', 'get_variant_details');
add_action('wp_ajax_nopriv_get_variant_details', 'get_variant_details');
...
function get_variant_details()
{
    ...
    $aggregated_user_rating_response = get_aggregated_user_rating($variant_id);
...
function get_interior_exterior_images_of_variant($variant_id)
{
    global $wpdb;
    $sql = $wpdb->prepare(
...
function get_aggregated_user_rating($variant_id)
{
