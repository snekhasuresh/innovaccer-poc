...
class WP_CarDealer_Taxonomy_Car_Color{
	...
	public static function definition() {
		$singular = __( 'Color', 'wp-cardealer' );
		$plural   = __( 'Colors', 'wp-cardealer' );
		...
		$rewrite_slug = get_option('wp_cardealer_listing_color_slug');
		if ( empty($rewrite_slug) ) {
			$rewrite_slug = _x( 'listing-color', 'Car color slug - resave permalinks after changing this', 'wp-cardealer' );
		}
		$rewrite = array(
			'slug'         => $rewrite_slug,
			'with_front'   => false,
		...
		register_taxonomy( 'listing_color', 'listing', array(
			'labels'            => apply_filters( 'wp_cardealer_taxomony_listing_color_labels', $labels ),
			'hierarchical'      => false,
			'parent_item'       => null,
...
WP_CarDealer_Taxonomy_Car_Color::init();