...
function enqueue_motor_faq_css()
{
    wp_enqueue_style('faq', get_stylesheet_directory_uri() . '/widget-shortcodes/individual-listing/css/faq.css');
}

add_shortcode('individual_listing_motor_faq', 'individual_listing_motor_faq_shortcode');
function individual_listing_motor_faq_shortcode($atts)
{
    enqueue_motor_faq_css();

    $listing_post = get_motor_listing_from_query_vars()['post'];
    ...
    ob_start();
    display_motor_faq($faqs->posts, $listing_post);
    return ob_get_clean();
}

function display_motor_faq($faq_posts, $listing_post)
{
    if (! empty($faq_posts)) {
?>
        <h2 class="title wa-title-text"><?php echo 'FAQ ' . $listing_post->post_title; ?></h2>
