...
class WP_CarDealer_Mixes {
    ...
    public static function get_image_mime_types() {
        return apply_filters( 'wp-cardealer-get-image-mime-types', array(
            'jpg'         => 'image/jpeg',
            'jpeg'        => 'image/jpeg',
            'jpe'         => 'image/jpeg',
            'gif'         => 'image/gif',
    ...
    public static function get_socials_network() {
        return apply_filters( 'wp-cardealer-get-socials-network', array(
            'fab fa-facebook-f' => esc_html__('Facebook', 'wp-cardealer'),
            'fab fa-x-twitter' => esc_html__('Twitter', 'wp-cardealer'),
            'fab fa-linkedin-in' => esc_html__('Linkedin', 'wp-cardealer'),
            'fab fa-dribbble' => esc_html__('Dribbble', 'wp-cardealer'),
            'fab fa-instagram' => esc_html__('Instagram', 'wp-cardealer'),
            'fab fa-google-plus-g' => esc_html__('Google +', 'wp-cardealer'),
            'fab fa-github-alt' => esc_html__('Github', 'wp-cardealer'),
            'fab fa-reddit-alien' => esc_html__('Reddit', 'wp-cardealer'),
            'fab fa-youtube' => esc_html__('Youtube', 'wp-cardealer'),
            'fab fa-vimeo-v' => esc_html__('Vimeo', 'wp-cardealer'),
            'fab fa-pinterest' => esc_html__('Pinterest', 'wp-cardealer'),
        ));
    }
    ...
    public static function get_all_countries() {
        ...
        return apply_filters( 'wp-cardealer-default-countries', $countries );
    }
    ...
    public static function get_listings_page_url() {
        if ( is_post_type_archive('listing') ) {
            ...
            $url = '';
            $taxs = ['type', 'category', 'color', 'location', 'condition', 'cylinder', 'door', 'drive_type', 'feature', 'fuel_type', 'make', 'model', 'offer_type', 'safety_type', 'transmission'];
            foreach ($taxs as $tax) {
            ...
            if ( is_page() && is_object($post) && basename( get_page_template() ) == 'page-listings.php' ) {
                ...
                $listings_page_id = wp_cardealer_get_option('listings_page_id');
        ...
        return apply_filters( 'wp-cardealer-get-listings-page-url', $url );
    }

    public static function get_dealers_page_url() {
        if ( is_post_type_archive('dealer') ) {
            $url = get_post_type_archive_link( 'dealer' );
        } elseif (is_tax()) {
            $url = '';
            $taxs = ['location', 'category'];
            foreach ($taxs as $tax) {
                if ( is_tax('dealer_'.$tax) ) {
                    global $wp_query;
                    $term = $wp_query->queried_object;
                    if ( isset( $term->slug) ) {
                        $url = get_term_link($term, 'dealer_'.$tax);
                    }
                }
            }
        } else {
            global $post;
            if ( is_page() && is_object($post) && basename( get_page_template() ) == 'page-dealers.php' ) {
                $url = get_permalink($post->ID);
            } else {
                $dealers_page_id = wp_cardealer_get_option('dealers_page_id');
                $dealers_page_id = self::get_lang_post_id($dealers_page_id, 'page');
                if ( $dealers_page_id ) {
                    $url = get_permalink($dealers_page_id);
                } else {
                    $url = get_post_type_archive_link( 'dealer' );
                }
            }
        }
        return apply_filters( 'wp-cardealer-get-dealers-page-url', $url );
    }

    public static function get_lang_post_id($post_id, $post_type = 'page') {
        return apply_filters( 'wp-cardealer-post-id', $post_id, $post_type);
    }
    ...
    public static function custom_pagination( $args = array() ) {
        ...
        $args = wp_parse_args( $args, array(
            'prev_text' => '<i class="flaticon-left-arrow"></i>'.esc_html__('Prev', 'wp-cardealer'),
            'next_text' => esc_html__('Next','wp-cardealer').'<i class="flaticon-right-arrow"></i>',
            'max_num_pages' => 10,
            'echo' => true,
            'class' => '',
        ));
        ...
        $pagination = apply_filters( 'wp-cardealer-custom-pagination', $pagination );
    ...
    public static function check_social_login_enable() {
        $facebook = WP_CarDealer_Social_Facebook::get_instance();
        $google = WP_CarDealer_Social_Google::get_instance();
        $linkedin = WP_CarDealer_Social_Linkedin::get_instance();
        $twitter = WP_CarDealer_Social_Twitter::get_instance();
    ...
    public static function social_login_after(){
        if ( self::check_social_login_enable() ) {
            echo '<div class="line-header"><span>'.esc_html__('or', 'wp-cardealer').'</span></div>';
            echo '</div></div>';
        }
    }
