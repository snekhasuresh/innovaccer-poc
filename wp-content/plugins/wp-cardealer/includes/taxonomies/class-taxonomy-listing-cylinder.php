...
class WP_CarDealer_Taxonomy_Car_Cylinder{
	...
	public static function definition() {
		$singular = __( 'Cylinder', 'wp-cardealer' );
		$plural   = __( 'Cylinders', 'wp-cardealer' );
		...
		$rewrite_slug = get_option('wp_cardealer_listing_cylinder_slug');
		if ( empty($rewrite_slug) ) {
			$rewrite_slug = _x( 'listing-cylinder', 'Car cylinder slug - resave permalinks after changing this', 'wp-cardealer' );
		}
		$rewrite = array(
			'slug'         => $rewrite_slug,
			'with_front'   => false,
		...
		register_taxonomy( 'listing_cylinder', 'listing', array(
			'labels'            => apply_filters( 'wp_cardealer_taxomony_listing_cylinder_labels', $labels ),
			'hierarchical'      => false,
			'parent_item'       => null,
...
WP_CarDealer_Taxonomy_Car_Cylinder::init();