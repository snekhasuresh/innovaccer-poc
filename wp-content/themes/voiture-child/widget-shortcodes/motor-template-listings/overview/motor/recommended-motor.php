<?php
function enqueue_single_listing_recommended_motor_css()
{
    ...
    wp_enqueue_style('single-listing-recommended_cars', get_stylesheet_directory_uri() . '/widget-shortcodes/motor-template-listings/overview/motor/css/single-listing-recommended-cars.css');
}


function single_listing_recommended_motor()
{
    enqueue_single_listing_recommended_motor_css();
    ...
    $listing_type_id = $post_meta['listing_type'][0];
    $listing_type_term = get_term_by('id', $listing_type_id, 'listing_type');
    $listing_type = $listing_type_term->name;

    $popular_bikes = get_popular_bikes_data(true);
    $latest_bikes = get_latest_bikes_data(true);

   	$term = get_term_by('name', $listing_make, 'motorcycle_make'); 
	$term_id = $term ? $term->term_id : null;
	$related_brand_motor_models = [];
	if ($term_id) {
		$related_brand_motor_models = get_posts(array(
			'post_type' => 'motorcycle-listing',
			'meta_query' => array(
				array(
					'key'     => 'make', // ACF field name
					'value'   => $term_id,         // Term ID to match
					'compare' => '=',              // Exact match
				),
			),
			'orderby' => 'date',
    ...
    $formatted_related_brand_motor_models = format_bike_response_by_posts($related_brand_motor_models, true);

    // $related_body_type_cars = get_posts(array(
    //     'post_type' => 'motorcycle-listing',
    //     'meta_query' => array(
    //         array(
    //             'key' => 'motorcycle-listing-type',
    ...
    //     'posts_per_page' => 10,
    // ));
    // $formatted_related_body_type_motors = format_bike_response_by_posts($related_body_type_cars, true);

    $upcoming_motors = get_posts(array(
        'post_type' => 'motorcycle-listing',
        'posts_per_page' => 10,
        'orderby' => 'date',
        'order' => 'DESC',
    ));
    $formatted_upcoming_motors = format_bike_response_by_posts($upcoming_motors, true);
    ...
    <div class="recommended-multi-car-tabs">
        ...
        <div class="recommended-multi-tab-content">
            <div id="recommended-multi-popular-content" class="recommended-multi-tab-pane">
                ...
                <?php display_recommended_motor($popular_bikes); ?>
            </div>
            <div id="recommended-multi-latest-content" class="recommended-multi-tab-pane">

                <?php display_recommended_motor($latest_bikes); ?>
            </div>
            <div id="recommended-multi-RM20k-80k-content" class="recommended-multi-tab-pane">

                <?php display_recommended_motor($formatted_related_brand_motor_models); ?>
            </div>
            <div id="recommended-multi-suv-content" class="recommended-multi-tab-pane">
                <?php display_recommended_motor($formatted_upcoming_motors); ?>
            </div>
        </div>
    </div>
    <script type="text/javascript">
        function toggleMotorVariants(carID) {
...
add_shortcode('single_listing_recommended_motor', 'single_listing_recommended_motor');

function display_recommended_motor($cars)
{
    if (empty($cars)) {
        return;
    }
    ...
    $top_car_model_data = get_option('top_bike_models', []);
